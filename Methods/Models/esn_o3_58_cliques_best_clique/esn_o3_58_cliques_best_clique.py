...
class esn_o3_58_cliques_best_clique(object):
	def delete(self):
		return 0
	...
	def train(self):
		...
		triangles = graph_cliques(0, 1, 3, node_number=self.reservoir_size, graph=W_c, degrees=degrees)
		pw_con = graph_cliques(0, 1, 2, node_number=self.reservoir_size, graph=W_c, degrees=degrees)
		print(f"4-Clique number = {len(clqs)}")
		print(f"3-Clique number = {len(triangles)}")
		...
		index = [clq for clq in range(len(clqs)) if clq not in [45]]
		print(index)
		clqs = np.delete(clqs, index, axis=0)
		print(np.array(clqs))
		clique_con_strengths = np.delete(clique_con_strengths, index)
