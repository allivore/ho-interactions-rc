...
class esn_o2(object):
	...
	def train(self):
		...
		clqs = graph_cliques(0, 1, 3, node_number=self.reservoir_size, graph=W_c, degrees=degrees)
		pw_con = graph_cliques(0, 1, 2, node_number=self.reservoir_size, graph=W_c, degrees=degrees)
		print(f"3-Clique number = {len(clqs)}")
		...
		for clique in clqs:
			for i in clique:
				for j in clique:
					if i != j:
						W_c[i, j] += 0.5*clique_con_strengths[num_i]
			num_i += 1
		W_h = W_h.toarray()
