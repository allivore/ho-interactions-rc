...
class esn_o3_eeg(object):
	...
	def __init__(self, params):
		...
		##########################################
		self.w_in_seed = params["w_in_seed"]
		##########################################
	...
	def train(self):
		...
		clique = rng.choice(self.reservoir_size, size=4, replace=False)
		print(f"Clique: {clique}")
		clique_con_strengths = rng.uniform(0.5, 2.5)
		print(f"Connection strength = {clique_con_strengths}")
		W_c = np.zeros((self.reservoir_size, self.reservoir_size))
		for i in clique:
			for j in clique:
				if i != j:
					W_c[i, j] += (1/3)*clique_con_strengths
		...
		rng_w_in = np.random.default_rng(self.w_in_seed)
		W_in = np.zeros((self.reservoir_size, input_dim))
		q = int(self.reservoir_size/input_dim)
		for i in range(0, input_dim):
			W_in[i*q:(i+1)*q,i] = self.sigma_input * (-1 + 2*rng_w_in.random(q))
	...
	def predictSequence(self, input_sequence, target_sequence):
		...
		for t in range(iterative_prediction_length):
			...
			i = input_sequence[t+dynamics_length].reshape((-1, 1))
			h = np.tanh(self.alpha_1*(W_h @ h) + W_in @ i + self.alpha_2*(W_c @ h))
		print("\n")
